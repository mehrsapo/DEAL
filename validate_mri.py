from batch_wrapper import validate
import argparse

def validate_mri(device='cuda:0'):
    modality = 'mri'
    method = 'deal'
    n_hyperparameters = 2
    ...
    for data_type in DATA_TYPES:
        for exp_name in EXP_NAMES:
            job_name = f"{modality}_coiltype_{coil_type}_acc_{acc}_cf_{cf}_noisesd_{noise_sd}_datatype_{data_type}_deal_{exp_name}_v2"
