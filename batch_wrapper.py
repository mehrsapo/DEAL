...
sys.path.append('../../')
sys.path.append('/home/pourya/all_methods/inverse_problems/mri')
...
def validate(method, modality, device, job_name, **kwargs):
    ...
    from mri.data.data_loader import get_dataloader
    if modality == "ct":
...
def test(method, modality, device, job_name, **kwargs):
    ...
    try:
       val_data = pd.read_csv(f"mri/validation_data/validation_scores_{job_name}.csv").reset_index(drop=True)
    except:
    ...
    # data loader
    sys.path.append(f'../{modality}/')
    sys.path.append(f'/home/pourya/all_methods/inverse_problems/mri')
    from mri.data.data_loader import get_dataloader
    if modality == "ct":
        noise_level = kwargs.get('noise_level', 2.0)
        data_loader = get_dataloader(mode, noise_level=noise_level)
    ...
    psnr_, psnr_std, ssim_, ssim_std, n_iter_, times = reconstruction_map_wrapper.batch_score(p1, p2)

    cols = ["p1", "p2", "job_name", "psnr", "psnr_std", "ssim", "ssim_std", "niter", "times"]

    data = [[p1, p2, job_name, psnr_, psnr_std, ssim_, ssim_std, n_iter_, times]]
    df = pd.DataFrame(data, columns=cols)

    df.to_csv(f"mri/test_data/{job_name}.csv")
...
class ReconstructionMap():
    ...
    def batch_score(self, p1, p2=None):
        ...
        times_ = torch.zeros(len(data_loader))
        ...
        if self.mode == "test":
            return(psnr_val.mean().item(), psnr_val.std().item(), ssim_val.mean().item(), ssim_val.std().item(), n_iter_val.mean().item(), times_.mean().item())
